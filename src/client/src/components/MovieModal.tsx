            ...
            <div className="space-y-3">
              ...
              >
                {movie.torrents.map((torrent, index) => (
                  <div key={index} className="flex items-center space-x-3 border rounded-lg p-3 hover:bg-accent cursor-pointer">
                    <RadioGroupItem value={index.toString()} id={`torrent-${index}`} />
                    <Label
                      htmlFor={`torrent-${index}`}
                      className="flex-1 cursor-pointer flex justify-between items-center"
                    >
                      <span className="font-medium">{torrent.quality} ({torrent.type})</span>
